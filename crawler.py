# -*- coding: utf-8 -*- 
import requests
import time
import pymysql
import json
import urllib.request
 
from datetime import datetime
from bs4 import BeautifulSoup

from urllib.parse import quote_plus    # 한글 텍스트를 퍼센트 인코딩으로 변환

from urllib.request import urlopen
from urllib.error import URLError, HTTPError

from selenium import webdriver
from selenium.webdriver.common.by import By

from selenium.webdriver.support.ui import WebDriverWait   # 해당 태그를 기다림
from selenium.webdriver.support import expected_conditions as EC
from selenium.common.exceptions import TimeoutException, NoSuchElementException,StaleElementReferenceException    # 태그가 없는 예외 처리
from selenium.webdriver.common.keys import Keys





options = webdriver.ChromeOptions()
options.add_argument('headless')
# options.add_argument('window-size=1920x1080')
options.add_argument("disable-gpu") # 혹은 options.add_argument("--disable-gpu")

options.add_argument('lang=ko_KR')    # 언어 설정
driver = webdriver.Chrome('./chromedriver', chrome_options=options)
driver.implicitly_wait(3)

wait = WebDriverWait(driver, 10)

# MySQL Connection 연결
conn = pymysql.connect(host='125.242.169.153', user='richman', password='richman', db='richman', charset='utf8') 

# css 체크 함수
def check_css(css_path, drv):
    try:
        drv.find_element_by_css_selector(css_path)
    except NoSuchElementException:
        return False
    return True



# 테이블 데이터 삭제
def delete_data():
    try:
        # Connection 으로부터 Cursor 생성
        curs = conn.cursor()

        # SQL문 실행
        sql = 'DELETE FROM tiger'
        curs.execute(sql)

        # # 데이타 Fetch
        curs.fetchall()
        conn.commit()
        conn.close

    except Exception:
        print('데이터 삭제 실패')

    finally:
        print('DB 삭제 완료')


# 테이블 데이터 삽입
def insert_data(data):
    try:
        # Connection 으로부터 Cursor 생성
        curs = conn.cursor()

        # SQL문 실행
        sql = 'INSERT INTO tiger (name, amount, per, price, diff, fluc) VALUES ("'+data[0]+'","'+data[1]+'","'+data[2]+'","'+data[3]+'","'+data[4]+'","'+data[5]+'")'
        
        curs.execute(sql)
        
        # # 데이타 Fetch
        curs.fetchall()
        conn.commit()
        conn.close

    except Exception:
        print('데이터 삽입 실패', data)

    finally:
        print('DB 삽입 완료:', data)

try:    # 정상 처리

    driver.get('https://finance.naver.com/item/main.nhn?code=292150')
    # url = 'data:image/jpeg;base64,'
    # url = 'http://img.khan.co.kr/news/2020/12/14/l_2020121501001726300141501.jpg'
    headers = {'User-Agent':'Chrome/87.0.4280.20'}

    # opener = urllib.request.URLopener()
    # opener.addheader('User-Agent', 'Chrome/87.0.4280.20')
    # opener.retrieve(url, 'test.png')

    # urllib.request.Request(url, headers=headers)

    # 리스트 개수 구하기
    tr = driver.find_elements_by_xpath('//*[@id="content"]/div[4]/table/tbody/tr')
    

    i = 0
    row = ''
    delete_data()
    
    while i < len(tr) :            
        i += 1
        
        data = []
        try:
            if(i == 1):
                css_path = f'#content > div.section.etf_asset > table > tbody > tr:nth-child({i}) > th'
            else : 
                css_path = f'#content > div.section.etf_asset > table > tbody > tr:nth-child({i}) > td'

            if check_css(css_path+':nth-child(1).ctg', driver) :
                td = driver.find_elements_by_css_selector(css_path)

                for idx in range(len(td)):
                    row += td[idx].text + '\t'
                    data.append(td[idx].text)
                    
                row += '\n'
                
                if(i != 1) :
                    insert_data(data)
            
        except Exception:
            print('데이터 없음')    
            
    print(row)
  
except TimeoutException:    # 예외 처리
    print('타임아웃 발생')

finally:    # 정상, 예외 둘 중 하나여도 반드시 실행
    driver.quit()
    

print('크롤링 끝')




